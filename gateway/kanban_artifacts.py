...
def _validate_image_structure(
    ...
    if not _image_container_ends_exactly(path, image_type, expected_size):
...
def _inspect(board: str, task_id: str, attachment: kanban_db.Attachment) -> tuple[dict[str, Any], Path]:
    ...
    if kind == "image":
        _validate_image_structure(path, mime_type, size)
...
def _collect_artifacts(
    board: str,
    task_id: str,
) -> tuple[list[tuple[dict[str, Any], Path]], int, int]:
    ...
    for attachment in candidates:
        if len(accepted) >= MAX_ARTIFACTS:
            ...
            continue
        try:
            metadata, path = _inspect(board, task_id, attachment)
        except KanbanApiError:
            rejected += 1
            continue
...
def list_artifacts(board: Any, task_id: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "complete": True,
        "task_id": task_id,
        "data": [metadata for metadata, _path in accepted],
...
def open_artifact(
    ...
    matched = next(
        ((metadata, path) for metadata, path in accepted if metadata["id"] == artifact_id),
        None,
    )
    if matched is None:
        raise KanbanApiError(404, "artifact_not_found", "Artifact was not found")
    expected, matched_path = matched
